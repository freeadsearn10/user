...
async def send_user_notification(
    ...
    try:
        ...
        if "Chat not found" in str(e):
            logger.warning(
                "Could not send notification to user %s: chat not found / not started",
                user_id,
            )
        elif "Forbidden: bot was blocked by the user" in str(e):
            logger.warning(
                "Could not send notification to user %s: bot blocked", user_id
            )
        else:
    ...
    try:
        ...
        with zipfile.ZipFile(zip_buffer, "w", zipfile.ZIP_DEFLATED) as zf:
            ...
            summary = {
                ...
                "approved_users": len(approved_users) - 1,
                "total_referrals": sum(
...
async def show_admin_commands(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Simple text-based admin help instead of complex inline panel."""
            ...
            if referrer_id and referrer_id != str(query.from_user.id):
                ...
                if success:
                    await query.edit_message_text(
                        get_text(
                            query.from_user.id, "referral_successful", referral_count=1
                        )
                    )
                    return
